...
} from '@/lib/hubspot/property-mappings';
...
import { Download, Filter, CreditCard, ExternalLink } from 'lucide-react';
...
function getPaymentStatus(order: Order): {
  ...
  if (!order.stripe_payment_intent_id) {
    ...
    return { status: 'No Payment', variant: 'secondary' };
  }

  // If we have payment intent ID, assume paid (would need API call for real status)
  if (
    ['Delivered', 'InTransit', 'PickedUp', 'Accepted', 'Assigned', 'ReadyForDispatch'].includes(
      order.status
    )
  ) {
...
export default function DispatcherClient({
  ...
  const [statusFilter, setStatusFilter] = useState<
    'all' | 'ReadyForDispatch' | 'Assigned' | 'Accepted' | 'PickedUp' | 'InTransit'
  >('all');
  ...
  const allSelectedOnPage = useMemo(
    () => filteredOrders.length > 0 && filteredOrders.every((o) => selectedIds[o.id]),
  ...
  const statusRowClass = (status: string) => {
    switch (status) {
      case 'ReadyForDispatch':
        return 'border-l-4 border-warning/60 bg-warning/5';
      case 'Assigned':
      case 'Accepted':
        return 'border-l-4 border-accent/60 bg-accent/5';
      case 'PickedUp':
      case 'InTransit':
        return 'border-l-4 border-secondary/60 bg-secondary/5';
      case 'Delivered':
        return 'border-l-4 border-success/60 bg-success/5';
      case 'Canceled':
        ...
        <div>
          <div className="flex items-center justify-between mb-6">
            <PageHeader
              ...
              breadcrumbs={[{ label: 'Home', href: '/' }, { label: 'Dispatcher' }]}
            />
            ...
            <CardContent className="p-4">
              <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-center">
                <div className="flex items-center gap-2 flex-1">
                  ...
                  <Input
                    placeholder="Search orders, address, customer, driver..."
                    value={search}
                    onChange={(e) => setSearch(e.target.value)}
                  />
                </div>
                <UiSelect value={statusFilter} onValueChange={(v: any) => setStatusFilter(v)}>
                  <UiSelectTrigger className="w-[220px]">
                ...
                <UiSelect value={settings.algorithm} onValueChange={(v: any) => setAlgorithm(v)}>
                  <UiSelectTrigger className="w-[220px]">
                    <UiSelectValue placeholder="Algorithm" />
                  </UiSelectTrigger>
                  <UiSelectContent>
                ...
                <Button
                  variant={isMapOpen ? 'secondary' : 'accent'}
                ...
                <div className="flex flex-col md:flex-row items-stretch md:items-center gap-2 mt-4">
                  ...
                  <div className="ml-auto flex items-center gap-2">
                    <UiSelect value={bulkDriver} onValueChange={setBulkDriver}>
                      <UiSelectTrigger className="w-[220px]">
                        <UiSelectValue placeholder="Assign to driver" />
                      </UiSelectTrigger>
                      <UiSelectContent>
                        {drivers.map((d) => (
                    ...
                    <Button
                      variant="accent"
                      disabled={!anySelected || !bulkDriver || isAssigning === 'bulk'}
                    ...
                    <Button
                      variant="secondary"
                      disabled={!anySelected || isAssigning === 'bulk'}
                      ...
                      <TableBody>
                        {filteredOrders.map((order) => (
                          <TableRow
                            ...
                            className={statusRowClass(order.status)}
                          >
                            <TableCell>
                            ...
                            <TableCell>
                              <div className="space-y-1">
                                <Badge
                                  variant={getPaymentStatus(order as Order).variant}
                                  className="text-xs"
                                >
                                  {getPaymentStatus(order as Order).status}
                                </Badge>
                                {(order as Order).stripe_payment_intent_id && (
                                  <a
                                    href={`https://dashboard.stripe.com/payments/${(order as Order).stripe_payment_intent_id}`}
                                    target="_blank"
                            ...
                            <TableCell className="text-right">
                              <div className="flex items-center justify-end gap-2">
                                ...
                                <SuggestionChips
                                  ...
                                  onPick={(driverId) => handleAssignDriver(order.id, driverId)}
                                />
                                {/* Contact actions */}
                                {order.customers?.phone && (
                                ...
                                <Button
                                  ...
                                  size="default"
                                  onClick={() =>
                                    handleAssignDriver(order.id, selectedDriver[order.id])
                                  }
          ...
          <Dialog
            open={!!orderForHubSpotDetails}
            onOpenChange={(open) => !open && setOrderForHubSpotDetails(null)}
          >
                ...
                <div className="space-y-6">
                      ...
                      items={[
                        {
                          label: 'Stage',
                          value:
                            hubspotMetadata?.pipelineStage ||
                            getDealPipelineForStatus(
                              orderForHubSpotDetails?.status || 'ReadyForDispatch'
                            ),
                        },
                        {
                          label: 'Delivery Status',
                          value:
                            hubspotMetadata?.deliveryStatus ||
                            getDeliveryStatusForStatus(
                              orderForHubSpotDetails?.status || 'ReadyForDispatch'
                            ),
                        },
                  ...
                  <MetadataCard
                    ...
                    items={[
                      { label: 'Route', value: hubspotMetadata?.deliveryRoute || 'Not set' },
                      {
                        label: 'Destination',
                        value:
                          hubspotMetadata?.deliveryLocation ||
                          orderForHubSpotDetails?.quotes?.dropoff_address ||
                          'Not set',
                      },
                      { label: 'Type', value: hubspotMetadata?.deliveryType || 'Standard' },
                      {
                        label: 'Weight Bracket',
                        value: hubspotMetadata?.weightBracket || 'Not set',
                      },
                      {
                        label: 'Special Instructions',
                        value: hubspotMetadata?.specialDeliveryInstructions || 'None',
                      },
                    ]}
                  />
                      ...
                      items={[
                        {
                          ...
                          value: formatDateTime(hubspotMetadata?.scheduledPickupTime),
                        },
                        {
                          label: 'Scheduled Delivery',
                          value: formatDateTime(hubspotMetadata?.scheduledDeliveryTime),
                        },
                        {
                          label: 'Actual Pickup',
                          value: formatDateTime(hubspotMetadata?.actualPickupTime),
                        },
                        {
                          label: 'Actual Delivery',
                          value: formatDateTime(hubspotMetadata?.actualDeliveryTime),
                        },
                      ]}
                    />
                    <MetadataCard
                      ...
                      items={[
                        ...
                        {
                          label: 'Recurring',
                          value: hubspotMetadata?.recurringFrequency || 'One-time',
                        },
                        {
                          label: 'Rush Requested',
                          value: hubspotMetadata?.rushRequested ? 'Yes' : 'No',
                        },
                        {
                          label: 'Services Proposed',
                          value: hubspotMetadata?.servicesProposed || 'Standard delivery',
                        },
                      ]}
                    />
                  </div>
                  ...
                  <MetadataCard
                    ...
                    items={[
                      {
                        label: 'Last synced',
                        value: formatDateTime(hubspotMetadata?.lastSyncedAt),
                      },
                      { label: 'Status', value: hubspotMetadata?.syncStatus || 'unknown' },
                      {
                        label: 'Warnings',
                        value: hubspotMetadata?.syncWarnings?.length
                          ? hubspotMetadata.syncWarnings.join(', ')
                          : 'None',
                      },
                      {
                        label: 'HubSpot Deal ID',
                        value:
                          hubspotMetadata?.dealId ||
                          orderForHubSpotDetails?.hubspot_deal_id ||
                          'Not linked',
                      },
                    ]}
                  />
                    ...
                    items={[
                      {
                        label: 'Status',
                        value: (
                          <Badge variant={getPaymentStatus(orderForHubSpotDetails!).variant}>
                            {getPaymentStatus(orderForHubSpotDetails!).status}
                          </Badge>
                        ),
                      },
                      {
                        label: 'Payment Intent',
                        value: orderForHubSpotDetails?.stripe_payment_intent_id ? (
                          <a
                            href={`https://dashboard.stripe.com/payments/${orderForHubSpotDetails.stripe_payment_intent_id}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-1 text-accent hover:underline"
                          >
                            {orderForHubSpotDetails.stripe_payment_intent_id.slice(0, 20)}...
                      ...
                      },
                      {
                        ...
                        value: orderForHubSpotDetails?.stripe_checkout_session_id ? (
                          <a
                            href={`https://dashboard.stripe.com/checkout/sessions/${orderForHubSpotDetails.stripe_checkout_session_id}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="flex items-center gap-1 text-accent hover:underline"
                          >
                            {orderForHubSpotDetails.stripe_checkout_session_id.slice(0, 20)}...
                            <ExternalLink className="h-3 w-3" />
                          </a>
                        ) : (
                          'Not set'
                      ...
                      {
                        label: 'Amount',
                        value: `$${orderForHubSpotDetails?.price_total.toFixed(2)} ${(orderForHubSpotDetails?.currency || 'USD').toUpperCase()}`,
                      },
                    ]}
                  />
                </div>
...
function exportCSV(orders: any[]) {
  ...
  const rows = orders.map((o) => [
    o.id.slice(0, 8),
    o.customers?.name || 'N/A',
    o.drivers?.name || 'Unassigned',
    o.status,
    `$${o.price_total?.toFixed(2) || '0.00'}`,
...
function SuggestionChips({
  ...
  return (
    <div className="flex items-center gap-1 mr-2">
      {suggested.map((d) => (
        <Button key={d.id} variant="secondary" size="sm" onClick={() => onPick(d.id)}>
          {d.name}
        </Button>
      ))}
    </div>
